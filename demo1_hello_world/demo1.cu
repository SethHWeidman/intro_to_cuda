...
__global__ void my_kernel_2D() {
  ...
  printf("rowID %d colID %d blockDim.x %d, blockDim.y %d gridDim.x %d gridDim.y %d numRows %d "
         "numCols %d myThreadID %d\n",
         rowID, columnID, blockDim.x, blockDim.y, gridDim.x, gridDim.y, numRows, numCols,
         myThreadID);
}
